...
export class CartService {
  ...
  setCount(count: number) {    //Cделаем отдельный метод, который мы будем вызывать для изменения показаний корзины.
    this.count = count;
    this.count$.next(this.count);  //Вызываем для того, чтобы оповестить наших слушателей о том что count изменился.
  }
