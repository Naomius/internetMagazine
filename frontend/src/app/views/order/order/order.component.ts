...
export class OrderComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.cartService.getCart()
      .subscribe((data: CartType | DefaultResponseType) => {
        ...
        this.calculateTotal(); //пересчитываем актуальное количество.
      });

      if (this.authService.getIsLoggedIn()) {  //Сделаем проверку на то, что пользователь авторизован.
        this.userService.getUserInfo()       //запрос на получение данных о пользователе
  ...
  creatOrder() {
    if (this.orderForm.valid && this.orderForm.value.firstName && this.orderForm.value.lastName && this.orderForm.value.phone
      ...
      const paramsObject: OrderTypeType = {
      ...
      this.orderService.creatOrder(paramsObject)
        .subscribe( {
          next: (data: OrderTypeType | DefaultResponseType) => {
            if ((data as DefaultResponseType).error !== undefined) {
              throw new Error((data as DefaultResponseType).message);
            }
            ...
            this.cartService.setCount(0); //оповестим наш хедер, о том что значение изменилось
          },
      ...
      this.orderForm.markAllAsTouched()  //помечаем все наши поля, как тронутые.
